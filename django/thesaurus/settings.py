...
config = AutoConfig(
    search_path="/run/secrets/"
)  # .env file is injected by docker secrets
...
DATABASES = {
    DEFAULT_DB_ALIAS: {
        ...
        "OPTIONS": {"options": "-c search_path=public,audit"},
    }
}
...
# AUTHENTICATION_BACKENDS is set below after LDAP config
...
TIME_ZONE = config("TZ", default="Europe/Prague")
...
CAN_LOGIN_AS = lambda request, target_user: (
    request.user.is_superuser and not target_user.is_superuser
)
...
if DEBUG:
    ...
    INTERNAL_IPS = type(
        str("ContainsEverything"), (), {"__contains__": lambda *a: True}
    )()

###### LDAP
# https://github.com/etianen/django-python3-ldap
# When LDAP_HOST is empty, LDAP authentication is disabled entirely.

_LDAP_HOST = config("LDAP_HOST", cast=str, default="")
LDAP_ENABLED: bool = bool(_LDAP_HOST)

if LDAP_ENABLED:
    LDAP_AUTH_URL = (
        f"ldap://{_LDAP_HOST}:{config('LDAP_PORT', cast=str, default='389')}"
    )
    LDAP_AUTH_USE_TLS = False
    LDAP_AUTH_CONNECTION_USERNAME = config("LDAP_USERNAME", cast=str, default="")
    LDAP_AUTH_CONNECTION_PASSWORD = config("LDAP_PASSWORD", cast=str, default="")
...
LOCALE_MIDDLEWARE_IGNORE_URLS = (API_URL_PATTERN,)
